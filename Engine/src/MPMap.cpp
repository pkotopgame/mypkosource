...
void MPMap::CreateSkyDoom(D3DXVECTOR3 center, float radius, char* txPath, bool hemisphere)
{
	...
	if(UseShader())
	{
		...
		const auto v = D3DXVECTOR4(0, (m_txMoveSpeed -= 1.0f) / 10000 * SKYDOOM_TEXTURE_MOVESPEED, 0, 0);
		dev->SetVertexShaderConstant(4,&v,1); //add by jze pass current frame number to vertex shader 
