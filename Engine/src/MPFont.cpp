	...
	for (int n=0; n<nLen; n++ )
	{
...

		{
				...
				&&rc.top >= pclip->top && rc.bottom <= pclip->bottom)
			{
				...
				_pDev->SetVertexShaderConstant(9,  &v[0], 1);
				_pDev->SetVertexShaderConstant(10, &v[1], 1);
				_pDev->SetVertexShaderConstant(11, &v[2], 1);
				_pDev->SetVertexShaderConstant(12, &v[3], 1);
				_pDev->SetVertexShaderConstant(13, &v[4], 1);
				_pDev->SetVertexShaderConstant(14, &v[5], 1);
				_pDev->SetVertexShaderConstant(15, &v[6], 1);
				_pDev->SetVertexShaderConstant(16, &v[7], 1);
				//��������ȫ���ڲ���������ʾ
			}else 
			{
				//���� 
...
bool CMPFont::Draw3DText(char* szText,D3DXVECTOR3& vPos, D3DXCOLOR color,float fScale)
{
	...
	for (int n=0; n<nLen; n++ )
	{
		...
		if(!_bUseSoft)
		{
			...
			_pDev->SetVertexShaderConstant(15, &v[6], 1);
			_pDev->SetVertexShaderConstant(16, &v[7], 1);
			_pDev->SetVertexShaderConstant(17, &v[8], 1);
			_pDev->SetVertexShaderConstant(18, &v[9], 1);
			_pDev->SetVertexShaderConstant(19, &v[10], 1);
			_pDev->SetVertexShaderConstant(20, &v[11], 1);
		}
		else
		{
#ifdef USE_RENDER
