...
public:    
    ...
    enum VIEW_TYPE
	{
		...
		VIEW_UI ,         // ½çÃæÍ¨³£ÐèÒªµÄÆ½ÃæäÖÈ¾View
        VIEW_3DUI ,      // 3D½çÃæ
	};
...
protected:
    ...
    HWND _hWnd{ nullptr };
	//CD3DFont*				_pFont; // ÄÚ²¿±£ÁôÒ»¸öFont¶ÔÏó×öÒ»Ð©Êä³ö²Ù×÷D3DXMATRIXA16			_matWorld;
	...
	int                     _nWorldViewStartX;  // ÆÁÄ»ÉÏÏÔÊ¾ÓÎÏ·ÊÀ½çµÄÆðµãÎ»ÖÃ
    int                     _nWorldViewStartY;  
    int                     _nWorldViewWidth;   // ÆÁÄ»ÉÏÏÔÊ¾ÓÎÏ·ÊÀ½çµÄ³ß´ç
    int                     _nWorldViewHeight;
    DWORD _dwBackgroundColor{ D3DCOLOR_XRGB(128, 128, 128) }; // ÆÁÄ»±³¾°Çå³ýÑÕÉ«
    DWORD                   _dwClearFlag;
    ...
    BOOL _bClearZBuffer{ true };
	 ...
	 float					_fAspect;
     int _nCurViewType{ -1 }; // 避免SetCurrentView中重复设置
	...
	// FPS
    BOOL _bRealFPS{ false }; //实际能运行多快，就跑多快，适用机器较慢的，例如845集成显卡
    DWORD _dwFPS{ 0 };
    DWORD _dwFrameCnt{ 0 };
    DWORD _dwLastTick{ 0 };
