...
void CGameConfig::Load(const char* pszFileName)   //  ��kop.cfg
{
	...
	while (!in.eof())  //�Ƿ񵽴��ļ�β
	{
		...
		else if (strKey == "VipStatesEffect")  //VipStatesEffect
		{
			VipStatesEffect = Str2Int(strValue);
		}
		else if (strKey == "VIPEffectDummy")  //VIPEffectDummy change effect dummy postion for vip states
		{
			VIPEffectDummy = Str2Int(strValue);
		}//
		else if (strKey == "VIPEffectHeightOff")  //VIPEffectHeightOff change the highet for effect
		{
			VIPEffectHeightOff = Str2Int(strValue);
		}
		// End
}
	in.close(); // �ر��ļ�
